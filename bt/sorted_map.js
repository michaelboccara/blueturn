export default class SortedMap {
  ...
  *entries() {
  ...
  boundEntries(key) {
    let lo = 0, hi = this._keys.length - 1;
    let lower = -1, upper = this._keys.length;
    ...
    while (lo <= hi) {
      ...
      if (midKey < key) {
        ...
        // Exact match
        return [[midKey, this._values[mid]], [midKey, this._values[mid]]];
      }
    }

    const lowerEntry = lower >= 0 ? [this._keys[lower], this._values[lower]] : undefined;
    const upperEntry = upper < this._keys.length ? [this._keys[upper], this._values[upper]] : undefined;
